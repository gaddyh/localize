...
3. **Response** — did the text shown to the user faithfully reflect the tool result,
   or did it invent a slot / claim a booking that never happened?

Plus tool-choice and tool-result-status checks, and a **self-validation mode** that
proves the grader itself is trustworthy before you point it at a real model.
...
## What's in the box (5 files, ~2.1k LOC, no framework)
...
| `grading.py` | The engine. `grade(gold, observed)` walks the two trajectories step-by-step and files every discrepancy into a labeled bucket across all layers. Pure-Python aggregation, no deps. |
...
Requires Python 3.10+. Dependencies: `pydantic`, `rich`, `scikit-learn`.
...
- **The response layer is heuristic.** `must_not_contain` → `unfaithful_action_claim` vs
  `wrong_slot_confirmed` is inferred from the gold's forbidden values and a small marker
  list. It's deterministic and works here, but the airtight upgrade is an LLM-judge that
  takes the same `must_reflect` / `faithfulness` fields as its rubric.
...
- [ ] LLM-judge backend for the response layer.
